...
class PlaylistManager():
    ...
    def view_sort_playlist(self, playlist_title, sort_name=None):
        ...
        try:
            if sort_name:
                ...
                self.connector.cursor.execute(f"SELECT * FROM soundlist ORDER BY {sort_name}")
            else:
    ...
    def add_sound_into_playlist(self, sound_title, sound_playlist):
        ...
        if sound_title not in self.connector.sound_list or sound_playlist not in self.playlist_list:
            raise Exception(f'Invalid sound name: {sound_title}')
