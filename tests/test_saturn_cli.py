...
#             ])
    ...
    def test_transcode_command(self):
        ...
        with patch('saturn_cli.AudioSegment') as mock_audio_segment:
            with patch('saturn_cli.playback.play') as mock_play:
                self.parser.argv = ['', '', '--transcode', 'sounds/coffee.wav', 'sounds/coffee.mp3']
                self.parser.transcode_command()
                mock_audio_segment.from_file.assert_called_once_with('sounds/coffee.mp3', format='mp3')
        ...
        with patch('os.path.exists') as mock_exists:
            ...
            with patch('saturn_cli.sa.WaveObject') as mock_wave_obj:
                with patch('saturn_cli.playback.play') as mock_play:
                    self.parser.argv = ['', '', '--transcode', 'sounds/coffee.mp3', 'sounds/coffee.wav']
                    self.parser.transcode_command()
                    mock_wave_obj.from_wave_file.assert_called_once_with('sounds/coffee.wav')
                    mock_play.assert_called_once()
